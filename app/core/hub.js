  ...
  async function hubDoLoginReal() {
    ...
    try {
      ...
      try {
        ...
        if (cached?.auth_id === window.currentUser.id && cached?.rol) {
          perfil = cached;
      ...
      if (!perfil) {
        const { data: perfilByAuth } = await window.sb.from('usuarios')
          .select(PERFIL_COLS).eq('auth_id', window.currentUser.id).maybeSingle();
        perfil = perfilByAuth;
      }
        ...
        if (perfilByEmail) {
          ...
          try {
            await window.sb.rpc('upsert_usuario', {
              p_auth_id: window.currentUser.id,
            ...
            await window.sb.from('usuarios').update({ auth_id: window.currentUser.id, activo: true })
              .eq('id', perfilByEmail.id).catch(() => {});
          }
          perfil = { ...perfilByEmail, auth_id: window.currentUser.id };
