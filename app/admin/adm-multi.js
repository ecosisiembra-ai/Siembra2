...
ADM.init = async function() {
  ...
  const _nomEsc = ADM.escuelaNombre || window._escuelaCfg?.nombre || ADM.escuelaCct || '';
  if (escuelaTag && _nomEsc) escuelaTag.textContent = _nomEsc;
...
ADM.renderDocentes = function() {
  ...
  const rolIcon  = { docente:'👨‍🏫', coordinador:'📋', ts:'🧠', prefecto:'👮', subdirector:'🏫', tutor:'🎓', admin:'⚙️', director:'🏛️', medico:'🩺', contralor:'📊', orientador:'🎯' };
  const rolColor = { docente:'adm-badge-blue', coordinador:'adm-badge-amber', ts:'adm-badge-green', prefecto:'adm-badge-red', subdirector:'adm-badge-blue', tutor:'adm-badge-green', admin:'adm-badge-amber', director:'adm-badge-green', medico:'adm-badge-green', contralor:'adm-badge-amber', orientador:'adm-badge-blue' };
  const rolLabel = { docente:'Docente', coordinador:'Coordinador', ts:'Trab. Social', prefecto:'Prefecto', subdirector:'Subdirector', tutor:'Tutor', admin:'Admin', director:'Director/a', medico:'Médico', contralor:'Contralor', orientador:'Orientador' };
    ...
    <div class="adm-form-row">
      <div class="adm-form-group">
        ...
        <select id="adm-p-rol" onchange="admToggleRolFields(this.value)">
          ...
          <option value="admin">Admin / Secretaría</option>
          <option value="medico">Médico escolar</option>
...
ADM.guardarPersonal = async function() {
  ...
  const ROL_LABELS = {
    ...
    admin:'Admin / Secretaría', medico:'Médico escolar',
    contralor:'Contralor', orientador:'Orientador/a',
  };
          ...
          if (insertErr.message?.toLowerCase().includes('duplicate') || insertErr.code === '23505') {
            ...
            if (existingData?.id) {
              await sb.from('usuarios').update({
                ...
                rol, activo: true, nivel: nivelPersonal
